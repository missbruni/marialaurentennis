...
const BookingForm: React.FC<BookingFormProps> = ({ bookingRef }) => {
  ...
  const availabilityForDate = React.useMemo(() => {
  ...
  return (
    <section ref={bookingRef} className="bg-gray-50 min-h-screen w-full">
      <div className="flex w-full p-24">
        <div className="flex-1 p-2">
          ...
          <h6 className="text-sm md:text-base text-gray-800">
            ...
            refine your technique, our private tennis lessons are tailored to
            your level and goals. Book a session today and take the next step in
            your tennis journey.
          </h6>
        </div>
      ...
      <AvailableLessons
        availabilities={availabilityForDate}
        date={selectedDate}
      />
    </section>
  );
